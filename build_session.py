...
def main(argv=None):
    ...
    print(f"\n=== SESSION: {len(df)} shots over "
          f"{df['elapsed_min'].max():.1f} min ===")

    # fatigue trends
    trends = fatigue_trends(df)
    ...
    if "zone" in df.columns:
        ...
        zone = df.groupby("zone").agg(agg).round(1)
    ...
    c = chart(df, os.path.join(args.out, "session_chart.png"))
    if c:
        print(f"\nchart: {c}")
